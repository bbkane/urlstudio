...
function hydrateFromAppUrl() {
    ...
    state = {
        ...
        editablePath: normalizePath(fromUrl.editablePath || parsedUser.editablePath),
...
function renderEditableRows() {
    ...
    state.editableRows.forEach((row, index) => {
        ...
        const isContinuation = Boolean(prevRow && hasKey && prevRow.key === row.key);
        ...
        addButton.addEventListener("click", () => {
            const currentRow = state.editableRows.find((candidate) => candidate.id === row.id);
...
function updateRowValue(id, field, value) {
    state = {
        ...state,
        editableRows: state.editableRows.map((row) => (row.id === id ? { ...row, [field]: value } : row)),
    };
    renderDerivedOutputs();
    syncAppUrl();
}
