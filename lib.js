...
export function parseUserUrlIntoState(userUrl) {
  ...
  try {
    ...
    const editableRows =
      rows.length > 0
        ? rows.map((row) => createQueryRow(row.key, row.value))
        : [createQueryRow("", "")];
    ...
    return {
      userUrl: trimmed,
      userUrlError:
        "Enter a valid absolute URL (for example https://example.com/path).",
      originalOrigin: "",
      originalPath: "",
      originalRows: [],
      editableOrigin: "",
...
export function buildGeneratedUrl(origin, path, rows) {
  ...
  return query
    ? `${trimmedOrigin}${normalized}?${query}`
    : `${trimmedOrigin}${normalized}`;
}
