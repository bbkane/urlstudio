...
test("parseUserUrlIntoState extracts URL pieces", () => {
  const next = parseUserUrlIntoState(
    "https://example.com/path?key=value&otherkey=othervalue",
  );
...
test("buildGeneratedUrl builds URL with query rows in insertion order", () => {
  ...
  assert.equal(
    buildGeneratedUrl("https://example.com", "/newpath", rows),
    "https://example.com/newpath?b=2&a=1",
  );
});
...
test("buildGeneratedUrl omits query when only blank default row exists", () => {
  ...
  assert.equal(
    buildGeneratedUrl("https://example.com", "/", rows),
    "https://example.com/",
  );
});
...
test("serializeAppStateToQuery omits default blank row when only origin/path are set", () => {
  ...
  state.editablePath = "/";
  assert.equal(
    serializeAppStateToQuery(state),
    "u=https%3A%2F%2Fexample.com&origin=https%3A%2F%2Fexample.com&path=%2F",
  );
});

test("parseAppStateFromSearch rebuilds paired qk/qv rows", () => {
  const parsed = parseAppStateFromSearch(
    "?qk=key&qv=value&qk=otherkey&qv=othervalue",
  );
  assert.deepEqual(
